program test
  ...
  real, allocatable, dimension (:,:) :: A, B, C, Cref
  ...
  call sgemm('N','T', 5, 3, 7, 1.0, A, 5, B, 3, 0.0, Cref, 5)

  context = f77_zmq_ctx_new()
  socket  = f77_zmq_socket(context, ZMQ_REQ)
  rc      = f77_zmq_connect(socket, 'tcp://localhost:5555')

  rc = f77_zmq_send(socket, 'sgemm', 5, ZMQ_SNDMORE)
  args = 'N,T,5,3,7,1.0,0.0'
  rc = f77_zmq_send(socket, args, len(trim(args)),  ZMQ_SNDMORE)
  rc = f77_zmq_send8(socket, A, 4_8*size(A,1)*size(A,2), ZMQ_SNDMORE)
  rc = f77_zmq_send8(socket, B, 4_8*size(B,1)*size(B,2), 0)
  rc = f77_zmq_recv8(socket, C, 4_8*size(C,1)*size(C,2), 0)
